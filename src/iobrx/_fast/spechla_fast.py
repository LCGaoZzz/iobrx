"""spechla_fast: verbatim port of the ``iobrpy.SpecHLA.SpecHLA`` SpecHLA
wrapper (CLI ``python -m iobrpy.main spechla -n NAME -1 R1 -2 R2 -o OUTDIR
[-j THREADS] [-u {0,1}]``, IOBRpy 0.2.0 gold standard).

Contract with the original
--------------------------
Every orchestration function below (``run_cmd``, PATH helpers,
``ensure_python_module`` / ``ensure_python_deps``, the conda probes,
``ensure_bcftools_121``, ``ensure_external_tools``, ``ensure_spechap_built``,
``ensure_bowtie2_index`` / ``detect_bowtie2_build``, ``run_spechla_rnaseq``,
``merge_hla_results``, ``parse_args`` and the ``main`` flow) is a
BYTE-VERBATIM copy of ``iobrpy/SpecHLA/SpecHLA.py`` — same protocol lines,
same candidate orders (``$BCFTOOLS`` -> bundled ``SpecHLA/bin/bcftools`` ->
PATH; ``$BOWTIE2_BUILD`` -> PATH), same conda auto-install paths, same
``sys.exit`` codes. The external binaries (bwa / bowtie2 / freebayes /
samtools / bgzip / tabix / bcftools / bam / blastn / perl / SpecHap /
ExtractHAIRs) are scheduled exactly as upstream: SpecHLA_RNAseq.sh ->
SpecHLA.sh unchanged from the SpecHLA asset tree.

The ONLY port-side differences:

1. ``detect_spec_hla_root(override=None)`` — upstream takes
   ``dirname(__file__)`` (it lives INSIDE the iobrpy package, next to
   ``script/`` + ``db/`` + ``bin/``). iobrx is a separate package, so the
   equivalent resolution is: explicit ``spec_hla_root=`` argument >
   ``$SPECHLA_ROOT`` env var > the ``SpecHLA/`` directory of the installed
   ``iobrpy`` package (found via ``importlib``). The printed protocol line
   is identical.
2. ``main(argv=None, spec_hla_root=None)`` threads that override into
   ``detect_spec_hla_root``; the flow/order of every step is unchanged.
3. ``print_colorful_message`` is imported from iobrpy when available, with
   a plain-text fallback matching the upstream non-TTY behaviour (banner
   bytes in redirected logs are identical either way).
4. ``spechla_original(argv)`` — escape hatch running the untouched upstream
   ``iobrpy.SpecHLA.SpecHLA.main`` (used by ``iobrx.spechla(backend=
   'python')``).

No periphery is re-implemented "faster": sample/command/merge logic here is
pure text I/O measured in milliseconds next to hours of external-tool
phasing (orchestration ceiling speedup ~ 1), so verbatim fidelity wins.
"""
...
try:  # pragma: no cover - optional dependency
    ...
    def print_colorful_message(text, color="cyan", bold=False, stream=None):
        """Plain-text fallback matching the upstream non-TTY behaviour."""
        stream = stream or sys.stdout
        newline = "" if text.endswith("\n") else "\n"
        stream.write(text + newline)

__all__ = ["main", "spechla_original", "detect_spec_hla_root",
           "ensure_python_deps", "ensure_external_tools",
           "ensure_spechap_built", "ensure_bowtie2_index",
           "detect_bowtie2_build", "run_spechla_rnaseq", "merge_hla_results",
           "parse_args", "print_colorful_message"]
...
# Detect SpecHLA root
# ------------------------------------------------------------
def _resolve_spec_hla_root(override=None, quiet=False):
    """
    Resolve the SpecHLA asset root (script/ + db/ + bin/).

    Upstream lives INSIDE the iobrpy package and simply takes
    ``dirname(__file__)``. iobrx is a separate package, so the faithful
    equivalent resolution order is:

    1. explicit ``override`` (API parameter ``spec_hla_root=``);
    2. the ``SPECHLA_ROOT`` environment variable;
    3. the ``SpecHLA/`` directory of the installed ``iobrpy`` package
       (the same tree upstream would resolve to), validated by the
       presence of ``script/whole/``.
    ...
    root = None
    if override:
        root = os.path.abspath(os.path.expanduser(str(override)))
    else:
        env_root = os.environ.get("SPECHLA_ROOT")
        if env_root:
            root = os.path.abspath(os.path.expanduser(env_root))
        else:
            try:
                import importlib.util
                spec = importlib.util.find_spec("iobrpy")
                locations = list(spec.submodule_search_locations or []) if spec else []
            except Exception:
                locations = []
            for loc in locations:
                cand = os.path.join(loc, "SpecHLA")
                if os.path.isdir(os.path.join(cand, "script", "whole")):
                    root = cand
                    break
    if root is None or not os.path.isdir(root):
        print(
            "[SpecHLA] ERROR: cannot locate the SpecHLA asset tree "
            "(script/ + db/ + bin/).\n"
            "[SpecHLA]        Pass spec_hla_root=<path>, set $SPECHLA_ROOT, or "
            "install IOBRpy (the SpecHLA assets ship inside the iobrpy "
            "package).",
            file=sys.stderr,
        )
        sys.exit(1)
    if not quiet:
        print(f"[SpecHLA] Using SpecHLA root: {root}")
    return root


def detect_spec_hla_root(override=None):
    """
    Detect the root directory of SpecHLA (upstream protocol line included).

    See :func:`_resolve_spec_hla_root` for the resolution order; with no
    override this resolves to the ``SpecHLA/`` directory of the installed
    ``iobrpy`` package — the same tree the upstream ``dirname(__file__)``
    strategy points at.
    ...
    str
        Absolute path to SpecHLA root directory.
    """
    return _resolve_spec_hla_root(override, quiet=False)
...
def main(argv=None, spec_hla_root=None):
    """
    Main entry point for the SpecHLA wrapper.

    ``spec_hla_root`` optionally overrides the SpecHLA asset-tree
    resolution (see :func:`_resolve_spec_hla_root`); the step order and
    every protocol line are the upstream ones.
    """
...
def spechla_original(argv=None):
    """Run the untouched upstream ``iobrpy.SpecHLA.SpecHLA.main(argv)``.
    Escape hatch for ``backend='python'``; propagates the upstream
    ``SystemExit`` (the iobrx public wrapper catches it and returns the
    code)."""
    from iobrpy.SpecHLA.SpecHLA import main as _orig_main

    return _orig_main(argv)


if __name__ == "__main__":
    # Mirrors `python -m iobrpy.main spechla ...` (upstream entry).
    main()
